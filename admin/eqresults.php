...
mysql_select_db($database_db_conn, $db_conn);
$query_shw_feedback = sprintf("SELECT * FROM eqresults WHERE userid = %s", GetSQLValueString($colname_shw_feedback, "text"));
...
?>
    ...
    <td width="729" style="text-align:left; vertical-align:top; padding-top:10px; padding-left:10px;"><form id="form1" name="form1" method="get" action="eqresults.php">
...
?>
        ...
        <tr>
          <td>&nbsp;</td>
          <td><input type="submit" name="button" id="button" value="Show Eq Results" /></td>
        </tr>
      </table>
    </form>
    <br />
    <table width="678" border="1" cellpadding="0" cellspacing="0">
              <tr>
                <td colspan="6"><?php 
				if($_GET['userid']!="")
				 {
				echo("&nbsp;".$_GET['userid']."&nbsp;EQ Results.");
				 }
				?></td>
                              </tr>
<tr>
		...
		do { ?>
          ...
          <td width="252">&nbsp;&nbsp;<?php echo $row_shw_feedback['quest']; ?></td>
          <td width="133">&nbsp;&nbsp;<?php echo $row_shw_feedback['ans']; ?></td>
          <td width="133"><?php echo $row_shw_feedback['cans']; ?></td>
          <td width="133"> <?php 
		  if($row_shw_feedback['ans']==$row_shw_feedback['cans'])
		   {
			   $mks=$mks+$row_shw_feedback['marls'];
			   echo("Correct");
		   }
		   else { echo("In Correct"); } 
		  ?>
		  </td>
          <td width="133"><?php echo $row_shw_feedback['marls']; ?></td>
                </tr>
